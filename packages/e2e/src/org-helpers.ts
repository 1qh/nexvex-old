...
const api = anyApi as unknown as typeof BeApi,
  ...
  tc = {
    ...
    mutation: async <F extends FunctionReference<'mutation'>>(
      f: F,
      args: FunctionArgs<F>
    ): Promise<FunctionReturnType<F>> => getClient().mutation(f, args),
  ...
  createTestUser = async (email: string, name: string) =>
