          ...
          try {
            ...
            if (errors.length > 0 && errorHandler) {
              ...
              if (errors.length > 1) {
                ...
                console.error(`[lazyconvex] Bulk operation: ${errors.length} of ${items.length} items failed`)
                for (let i = 1; i < errors.length; i += 1) console.error(`[lazyconvex] Bulk error ${i + 1}:`, errors[i]) // eslint-disable-line no-console
              }
            }
            if (results.length > 0) {
              if (toastCfg?.success) {
