...
describe('Zod integration', () => {
  test('unwrapZod handles optional wrapper', async () => {
    const { unwrapZod } = await import('lazyconvex/zod'),
  ...
  test('cvFileKindOf detects file meta', async () => {
    const { cvFileKindOf } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4'),
  ...
  test('type detection for all primitive types', async () => {
    const { isArrayType, isBooleanType, isDateType, isNumberType, isStringType } = await import('lazyconvex/zod')
  ...
  test('enumToOptions transforms enum schema', async () => {
    const { enumToOptions } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4'),
  ...
  test('requiredPartial creates partial with required fields', async () => {
    const { requiredPartial } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4'),
      testSchema = z.object({
        email: z.string(),
        id: z.string(),
  ...
  test('elementOf extracts array element schema', async () => {
    const { elementOf, unwrapZod } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4'),
...
describe('Zod introspection snapshots', () => {
  test('defaultValues returns correct defaults for all types', async () => {
    const { defaultValues } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4'),
      testSchema = z.object({
        active: z.boolean(),
        age: z.number(),
  ...
  test('coerceOptionals trims and nullifies empty optional strings', async () => {
    const { coerceOptionals } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4'),
      testSchema = z.object({
        bio: z.string().optional(),
        name: z.string()
  ...
  test('pickValues extracts matching fields with defaults for missing', async () => {
    const { pickValues } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4'),
      testSchema = z.object({
        active: z.boolean(),
        age: z.number(),
  ...
  test('isOptionalField detects optional in nested wrappers', async () => {
    const { isOptionalField } = await import('lazyconvex/zod'),
      { z } = await import('zod/v4')
