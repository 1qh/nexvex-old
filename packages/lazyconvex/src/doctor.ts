...
const bold = (s: string) => `\u001B[1m${s}\u001B[0m`,
  ...
  checkEslintContent = (content?: string): CheckResult => {
    ...
    if (content.includes('lazyconvex/eslint'))
      return { details: ['lazyconvex/eslint plugin configured'], status: 'pass', title: 'ESLint Configuration' }
    return {
      details: ['eslint.config found but lazyconvex/eslint not imported'],
      status: 'warn',
  ...
  checkDeps = (pkg?: Record<string, unknown>): CheckResult => {
    ...
    const deps = (pkg.dependencies ?? {}) as Record<string, string>,
      ...
      all = { ...deps, ...devDeps },
      details: string[] = [],
      required = ['convex', 'zod', 'lazyconvex']
    let missing = 0
