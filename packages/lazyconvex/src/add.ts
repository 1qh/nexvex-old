...
const TABLE_TYPES = new Set<TableType>(['cache', 'child', 'org', 'owned', 'singleton']),
  ...
  printAddHelp = () => {
    console.log(`${bold('lazyconvex add')} — add a new table/endpoint to your project\n`)
    console.log(bold('Usage:'))
    console.log('  lazyconvex add <table-name> [options]\n')
    ...
    console.log(`  ${dim('$')} lazyconvex add todo --fields="title:string,done:boolean"`)
    console.log(
      `  ${dim('$')} lazyconvex add wiki --type=org --fields="title:string,content:string,status:enum(draft,published)"`
    )
    console.log(`  ${dim('$')} lazyconvex add message --type=child --parent=chat --fields="text:string"`)
    console.log(`  ${dim('$')} lazyconvex add profile --type=singleton --fields="displayName:string,bio:string?"`)
    console.log(`  ${dim('$')} lazyconvex add movie --type=cache --fields="title:string,tmdb_id:number"\n`)
  },
  ...
  genSchemaContent = (name: string, type: TableType, fields: ParsedField[]): string => {
    ...
    if (type === 'child')
      return `import { child } from 'lazyconvex/schema'
...
`
    ...
    return `import { ${importFn} } from 'lazyconvex/schema'
import { ${sortedImports.join(', ')} } from 'zod/v4'
